...
library FundManagementLogic {
    ...
    struct CalculateActualFundsVars {
        ...
        uint256 maturity;
        uint256 defaultOrderBookMaturity;
    ...
    function convertFutureValueToGenesisValue(
        ...
        (int256 removedAmount, int256 currentAmount, bool isAllRemoved) = IFutureValueVault(
            futureValueVault
        ).reset(_maturity, _user);
        ...
        if (removedAmount != 0) {
            ...
            if (isAllRemoved && Storage.slot().pendingOrderAmounts[_ccy][_maturity] == 0) {
    ...
    function updateFunds(
        ...
        if (_feeInFV > 0) {
            ...
            IFutureValueVault(futureValueVault).increase(_maturity, reserveFundAddr, _feeInFV);
    ...
    function getActualFunds(
        ...
        vars.defaultOrderBookMaturity = Storage.slot().orderBookMaturities[_ccy][0];
        vars.minDebtUnitPrice = _minDebtUnitPrice;

        if (_maturity == 0) {
            vars.isTotal = true;
            vars.maturity = vars.defaultOrderBookMaturity;
            vars.isDefaultMarket = true;
        } else {
            vars.isTotal = false;
            vars.maturity = _maturity;
            vars.isDefaultMarket = vars.maturity == vars.defaultOrderBookMaturity;
        }
        ...
        for (uint256 i; i < vars.maturities.length; i++) {
            ...
            bool isMatured = vars.defaultOrderBookMaturity > currentMaturity;

            if (vars.isDefaultMarket || currentMaturity == vars.maturity) {
                {
                    bool isDefaultMarket = currentMaturity == vars.defaultOrderBookMaturity;
    ...
    function cleanUpFunds(
        ...
        for (uint256 i; i < maturities.length; i++) {
            uint256 activeMaturity = maturities[i];
    ...
    function _cleanUpOrders(
        ...
        ) = ILendingMarket(Storage.slot().lendingMarkets[_ccy]).cleanUpOrders(_maturity, _user);
    ...
    function _getFundsFromFutureValueVault(
        ...
        int256 futureValueInMaturity = vars.futureValueVault.getBalance(currentMaturity, _user);
        ...
        if (futureValueInMaturity != 0) {
            if (isMatured) {
                ...
                if (vars.isTotal && !isDefaultMarket) {
                    ...
                    uint256 unitPrice = vars.market.getMarketUnitPrice(vars.maturity);
    ...
    function _getFundsFromInactiveBorrowOrders(
        ...
        (funds.workingOrdersAmount, funds.borrowedAmount, filledFutureValue) = vars
            .market
            .getTotalAmountFromBorrowOrders(currentMaturity, _user, currentMinDebtUnitPrice);
    ...
    function _getFundsFromInactiveLendOrders(
        ...
        uint256 filledFutureValue;
        (funds.workingOrdersAmount, funds.lentAmount, filledFutureValue) = vars
            .market
            .getTotalAmountFromLendOrders(currentMaturity, _user);
            ...
            if (isMatured) {
                ...
                if (vars.isTotal && !isDefaultMarket) {
                    (funds.presentValue, funds.futureValue) = _convertFVtoOtherMaturity(
                        ...
                        currentMaturity,
                        filledFutureValue.toInt256(),
                        vars.market.getMarketUnitPrice(vars.defaultOrderBookMaturity)
                    );
                } else if (vars.isTotal || !vars.isDefaultMarket || isDefaultMarket) {
                    funds.futureValue = filledFutureValue.toInt256();
                    funds.presentValue = _calculatePVFromFV(
    ...
    function _convertFVtoOtherMaturity(
        ...
        if (
            ...
            presentValue = _calculatePVFromFV(_market, _fromMaturity, _fromFutureValue);
    ...
    function _resetFundsPerMaturity(
        ...
        ).executeForcedReset(_maturity, _user, _amount);

        int256 remainingAmount = _amount - totalRemovedAmount;

        bool isDefaultMarket = _maturity == Storage.slot().orderBookMaturities[_ccy][0];
    ...
    function _calculatePVFromFV(
        ...
        uint256 unitPrice = _market.getMarketUnitPrice(_maturity);
