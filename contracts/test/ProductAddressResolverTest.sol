...
contract ProductAddressResolverTest is ProductAddressResolver {
    ...
    function getGasCostOfGetProductContractWithTypeConversion(bytes32 _dealID) external view returns (uint256) {
        uint256 gasBefore = gasleft();
        bytes4 _prefix;
    ...
    function getGasCostOfGetControllerContractWithTypeConversion(bytes32 _dealID) external view returns (uint256) {
        uint256 gasBefore = gasleft();
        bytes4 _prefix;
        assembly {
