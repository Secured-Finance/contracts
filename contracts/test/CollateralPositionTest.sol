...
contract CollateralPositionTest {
    ...
    function deposit(address depositor, address counterparty, uint256 amount) public {
    ...
    function withdraw(address user, address counterparty, uint256 amount) public {
        (uint256 lockedCollateralBeforeUser, ) = get(user, counterparty);

        CollateralPosition.withdraw(
            _positions, 
            user, 
            counterparty,
            amount
        ...
        uint256 withdrawn = lockedCollateralBeforeUser >= amount ? amount : lockedCollateralBeforeUser;
        require(
    ...
    function liquidate(address from, address to, uint256 amount) public {
        (
            uint256 lockedCollateralBeforeFrom, 
            uint256 lockedCollateralBeforeTo
        ) = get(from, to);

        CollateralPosition.liquidate(
            _positions, 
            from, 
            to,
            amount
        ...
        uint256 liquidated = lockedCollateralBeforeFrom >= amount ? amount : lockedCollateralBeforeFrom;
    ...
    function rebalance(
        ...
        uint256 rebalanced = lockedCollateralBeforeUser0 >= amount ? amount : lockedCollateralBeforeUser0;
