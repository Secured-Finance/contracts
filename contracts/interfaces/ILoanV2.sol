...
interface ILoanV2 {
    ...
    event Register(address indexed lender,address indexed borrower,bytes32 ccy,uint256 term,uint256 notional,uint256 rate,bytes32 indexed dealId);
    ...
    function addLendingMarket(bytes32 _ccy,uint256 _term,address addr) external;
    ...
    function getDealLastPV(address party0,address party1,bytes32 loanId) external view returns (uint256, uint256);
    ...
    function register(address maker,address taker,uint8 side,bytes32 ccy,uint256 term,uint256 notional,uint256 rate) external returns (bytes32 loanId);
