...
interface ITermStructure {
    ...
    event TermAdded(uint256 numDays, uint256 dfFrac, uint256 numPayments);
    ...
    function getNumPayments(uint256 _numDays) external view returns (uint256);
    function getTerm(uint256 _numDays) external view returns (uint256, uint256, uint256);
    function getTermSchedule(uint256 _numDays) external view returns (uint256[] memory);
    ...
    function supportTerm(uint256 _numDays,uint256 _dfFrac,uint256 _numPayments,uint256[] memory _couponSchedule,bytes4[] memory _products,bytes32[] memory _currencies) external  returns (bool);
