...
contract CollateralAggregatorV2 is ICollateralAggregator, ProtocolTypes, CollateralManagement {
    ...
    */
    ...
    {
        return _netTotalAndHypotheticalPV(
            _party0, 
            _party1, 
            "", 0, 0, false
        );
    ...
    */
        ...
        bool _isSettled) internal view returns (uint256, uint256)
    {
        ...
        if (vars.net0 > 0) {
            vars.req0 = vars.minMarginReq0 > (vars.net0.mul(MARGINLEVEL)).div(BP) ? vars.minMarginReq0 : vars.net0;
        } else {
            vars.req0 = vars.minMarginReq0;
        }

        if (vars.net1 > 0) {
            vars.req1 = vars.minMarginReq1 > (vars.net1.mul(MARGINLEVEL)).div(BP) ? vars.minMarginReq1 : vars.net1;
        } else {
    ...
    */
    ...
    )
    {
        ...
        (vars.colAdjustment0, vars.isWithdraw0) = _determineCollateralAdjustment(vars.lockedCollateral0, vars.targetReq0);
        (vars.colAdjustment1, vars.isWithdraw1) = _determineCollateralAdjustment(vars.lockedCollateral1, vars.targetReq1);
    ...
    */
    ...
    )
    {
        ...
        if (vars.coverage > MARGINLEVEL) { // TODO: discuss if it makes sense to decrease to 100%
    ...
    function _totalLockedCollateralInPosition(
        address _party0,
        address _party1
        ...
        for (uint256 i = 0; i < vars.len; i++) {
            ...
            (vars.lockedCollateral0, vars.lockedCollateral1) = ICollateralVault(vaultAddr).getLockedCollateralInETH(
                _party0, 
                _party1
            );
