...
library CollateralPosition {
    ...
    */
    ...
    function _handleFlippedCase(
        ...
        CollateralPosition.Position memory
    ) { 
        if (flipped) {
    ...
    */
    function withdraw(
        ...
        if (!flipped) {
            maxWithdraw = position.lockedCollateralA >= amount ? amount : position.lockedCollateralA;
            position.lockedCollateralA = position.lockedCollateralA.sub(maxWithdraw);
        } else {
            maxWithdraw = position.lockedCollateralB >= amount ? amount : position.lockedCollateralB;
    ...
    */
    function rebalance(
        ...
        address toParty,
        uint256 amount
    ) internal returns (uint256 rebalanced) { // max checks
