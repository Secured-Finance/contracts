...
find $CONTRACTS_FOLDER \( -path 'tmp/dependencies' -prune -o -path 'tmp/liquidators/interfaces' -prune -o -path 'tmp/mocks' -prune -o -path 'tmp/protocol/interfaces' -prune -o -path 'tmp/protocol/libraries' -prune -o -path 'tmp/protocol/mixins' -prune -o -path 'tmp/protocol/storages' -prune -o -path 'tmp/protocol/types' -prune -o -path 'tmp/protocol/utils' -prune \) -o -name '*.sol' -print | while IFS=$'\n' read -r FILE; do
