...
module.exports = async function ({ getNamedAccounts, deployments }) {
    ...
    const lendingMarket = await ethers.getContractAt("LendingMarket", '0x1723CA6fB3f9Bcd48c5aBBd8d393CE58aAa0c8F3');
    ...
    for (i = 0; i < sortedTermDays.length; i++) {
        ...
        console.log('Deployed FIL lending market with ' + sortedTermDays[i] + ' days term at ' + receipt.events[0].args.marketAddr);
        ...
        await (await lendingMarket.setCollateral(collateralAggregator.address, { from: deployer })).wait();
        await (await lendingMarket.setLoan(loanV2.address, { from: deployer })).wait();
        await (await collateralContract.addCollateralUser(lendingMarket.address, { from: deployer })).wait();
        await (await loanV2Contract.addLendingMarket(
            hexFILString,
            sortedTermDays[i],
