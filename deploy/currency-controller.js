...
module.exports = async function ({ getNamedAccounts, deployments }) {
    ...
    console.log('Deployed MockV3Aggregator FIL/ETH price feed at ' + filToETHPriceFeed.address);

    const ethToUSDPriceFeed = await deploy("MockV3Aggregator", {
        from: deployer,
        args: [18, hexETHString, ethToUSDRate.toString()]
    });
    console.log('Deployed MockV3Aggregator ETH/USD price feed at ' + ethToUSDPriceFeed.address);

    const btcToETHPriceFeed = await deploy("MockV3Aggregator", {
        from: deployer,
        args: [18, hexBTCString, btcToETHRate.toString()]
    });
    console.log('Deployed MockV3Aggregator BTC/ETH price feed at ' + btcToETHPriceFeed.address);

    await (await currencyControllerContract.supportCurrency(
        hexETHString,
        'Ethereum',
        60,
    ...
    await (await currencyControllerContract.updateCollateralSupport(hexETHString, true)).wait();
    await (await currencyControllerContract.updateMinMargin(hexETHString, 2500)).wait();
