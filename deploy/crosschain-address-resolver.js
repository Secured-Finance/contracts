module.exports = async function ({ getNamedAccounts, deployments }) {
    ...
    await (await collateralContract.setCrosschainAddressResolver(crosschainResolver.address)).wait();
    console.log('Deployed CrosschainAddressResolver at ' + crosschainResolver.address);

    const ethVaultContract = await ethers.getContractAt("CollateralVault", ethVault.address);

    let btcAddress = '3QTN7wR2EpVeGbjBcHwQdAjJ1QyAqws5Qt';
    let filAddress = 'f2ujkdpilen762ktpwksq3vfmre4dpekpgaplcvty';

    await (await collateralContract.functions['register(string[],uint256[])']([btcAddress, filAddress], [0, 461])).wait();
    await (await ethVaultContract.functions['deposit(uint256)']('1000000000000000000', { value: '1000000000000000000' })).wait()
}
