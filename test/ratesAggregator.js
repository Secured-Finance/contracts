...
contract('FXRatesAggregator', async (accounts) => {
  ...
  before(
    'deploy FXRatesAggregator and Mock Chainlink price feeds',
    async () => {
      fxRatesAggregator = await FXRatesAggregator.new();

      btcToUSDPriceFeed = await MockV3Aggregator.new(
        8,
        hexBTCString,
        5612587723563,
      );
      let setPriceFeedTx = await fxRatesAggregator.linkPriceFeed(
        3,
      ...
      setPriceFeedTx = await fxRatesAggregator.linkPriceFeed(
        1,
      ...
      btcToETHPriceFeed = await MockV3Aggregator.new(
        18,
        hexBTCString,
        web3.utils.toBN(23889912590000000000),
      );
      setPriceFeedTx = await fxRatesAggregator.linkPriceFeed(
        3,
      ...
      filToETHPriceFeed = await MockV3Aggregator.new(
        18,
        hexFILString,
        web3.utils.toBN(67175250000000000),
      );
      setPriceFeedTx = await fxRatesAggregator.linkPriceFeed(
        1,
  ...
  describe('Test getLastETHPrice and getHistoricalPrice for BTC in ETH', async () => {
    it('Succesfully get last price for BTC in ETH', async () => {
      await btcToETHPriceFeed.updateAnswer(
        web3.utils.toBN(23889912590000000000),
        { from: owner },
      );
  ...
  describe('Test getLastETHPrice and getHistoricalPrice for FIL in ETH', async () => {
    it('Succesfully get last price for FIL in ETH', async () => {
      await filToETHPriceFeed.updateAnswer(web3.utils.toBN(67175250000000000), {
        from: owner,
      });
  ...
  describe('Test convertion from FIL to ETH', async () => {
    it('Successfully convert 10,000 for FIL in ETH', async () => {
      let filInETH = await fxRatesAggregator.convertToETH(
        1,
