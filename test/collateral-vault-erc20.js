...
contract('ERC20 based CollateralVault', async (accounts) => {
    ...
    describe('Test token deposits into collateral vault', () => {
        ...
        it('Deposit all tFIL tokens into the vault by Alice, validate token balances', async () => {
            ...
            await tokenContract.approveInternal(alice, vault.address, aliceTokenBalance);
            await vault.connect(aliceSigner)['deposit(uint256)'](aliceTokenBalance.toString());
            ...
            independentCollateral.toString().should.be.equal(aliceLockedTokens.toString());

            aliceMaxWithdraw = await currencyController.convertToETH(hexFILString, aliceLockedTokens);
            await collateral.setMaxCollateralBookWidthdraw(alice, aliceMaxWithdraw);
            aliceMaxWithdraw.toString().should.be.equal(
        ...
        it('Try to deposit some tokens by Bob, expect revert on non-approved transfer', async () => {
            ...
            bobDepositAmt.toString().should.be.equal(
                (await tokenContract.balanceOf(bob)).toString()
            );
            bobTokenBalance = bobDepositAmt;

            await expectRevert( // expect revert on deposit without approve on token transfer
                vault.connect(bobSigner)['deposit(uint256)'](bobDepositAmt.toString()),
                "TransferHelper: TRANSFER_FROM_FAILED"
            );
        });
    ...
    describe('Test collateral vault rebalancing functionality', () => {
        it('Rebalance all independent collateral from Alice into the position with Bob', async () => {
            ...
            let lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, bob);
            lockedCollateral[0].toString().should.be.equal(rebalanceAmtTokens.toString());
            lockedCollateral[1].toString().should.be.equal('0');

            lockedCollateral = await vault['getLockedCollateral(address)'](alice);
            lockedCollateral.toString().should.be.equal(rebalanceAmtTokens.toString());
        ...
        it('Rebalance more collateral than deposited by Alice and Bob, expect no state changes', async () => {
            ...
            rebalanceAmtBob = await currencyController.convertToETH(hexFILString, rebalanceAmtBob);

            await collateral.rebalanceTo(alice, bob, rebalanceAmtAlice, vault.address);
            await collateral.rebalanceTo(bob, alice, rebalanceAmtBob, vault.address);

            let lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, bob);
            lockedCollateral[0].toString().should.be.equal(aliceLockedTokens.toString());
            lockedCollateral[1].toString().should.be.equal('0');
        ...
        it("Rebalance from Bob's position with Alice, expect no state changes as Bob deposited 0 tokens into position", async () => {
            ...
            await collateral.rebalanceFrom(bob, alice, rebalanceAmtBob, vault.address);

            let lockedCollateral = await vault['getLockedCollateral(address,address)'](bob, alice);
            lockedCollateral[0].toString().should.be.equal('0');
            lockedCollateral[1].toString().should.be.equal(aliceLockedTokens.toString());
        ...
        it("Rebalance between Bob's position with Alice to Carol, expect no state changes", async () => {
            ...
            await collateral.rebalanceBetween(bob, alice, carol, rebalanceAmtBob, vault.address);

            let lockedCollateral = await vault['getLockedCollateral(address,address)'](bob, alice);
            lockedCollateral[0].toString().should.be.equal('0');

            lockedCollateral = await vault['getLockedCollateral(address,address)'](bob, carol);
            lockedCollateral[0].toString().should.be.equal('0');
            lockedCollateral[1].toString().should.be.equal('0');
        });
        ...
        it("Rebalance between Alice's position with Bob to Carol, validate state changes", async () => {
            ...
            await collateral.rebalanceBetween(alice, bob, carol, rebalanceAmt, vault.address);

            let lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, bob);
            lockedCollateral[0].toString().should.be.equal(lockedAmtAlice.toString());
            lockedCollateral[1].toString().should.be.equal('0');

            lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, carol);
            lockedCollateral[0].toString().should.be.equal(lockedAmtAlice.toString());
            lockedCollateral[1].toString().should.be.equal('0');
        });
        ...
        it("Rebalance from Alice's position with Bob more than deposited by Alice, validate correct rebalance state changes", async () => {
            ...
            let independentCollateral = await vault.getIndependentCollateral(alice);
            independentCollateral.toString().should.be.equal(lockedAmtAlice.toString());
        });
        ...
        it("Rebalance between Alice's position with Bob to Carol more than deposited by Alice, validate correct rebalance state changes", async () => {
            ...
            let rebalanceAmt = aliceMaxWithdraw;
            aliceLockedInPositionWithCarol = aliceLockedTokens.div(toBN(2)).add(rebalanceAmtToTokens);

            await collateral.rebalanceTo(alice, bob, rebalanceAmtToETH, vault.address);
            let lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, bob);
            lockedCollateral[0].toString().should.be.equal(rebalanceAmtToTokens.toString());
            lockedCollateral[1].toString().should.be.equal('0');

            await collateral.rebalanceBetween(alice, bob, carol, rebalanceAmt, vault.address);
            lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, bob);
            lockedCollateral[0].toString().should.be.equal('0');
            lockedCollateral[1].toString().should.be.equal('0');

            lockedCollateral = await vault['getLockedCollateral(address,address)'](alice, carol);
            lockedCollateral[0].toString().should.be.equal(aliceLockedInPositionWithCarol.toString());
            lockedCollateral[1].toString().should.be.equal('0');
        });
    });
    ...
    describe('Test collateral vault liquidation functionality', () => {
        ...
        it('Liquidate all collateral from Alice to Carol, validate state changes', async () => {
            let liquidationAmt = await currencyController.convertToETH(hexFILString, aliceLockedInPositionWithCarol);
            ...
            let lockedCollateral = await vault['getLockedCollateral(address,address)'](carol, alice);
            lockedCollateral[0].toString().should.be.equal(carolLockedInPositionWithAlice.toString());
            lockedCollateral[1].toString().should.be.equal(aliceLockedInPositionWithCarol.toString());
    ...
    describe('Test collateral vault withdawal functionality', () => {
        ...
        it('Try to withdraw more collateral than provided by Alice, validate correct balance changes', async () => {
            ...
            let independentCollateral = await vault.getIndependentCollateral(alice);
            independentCollateral.toString().should.be.equal(aliceMaxWithdrawTokens.toString());

            let aliceTokenBalance = await tokenContract.balanceOf(alice);
            aliceTokenBalance.toString().should.be.equal('0');

            await vault.connect(aliceSigner)['withdraw(uint256)'](withdrawAmt.toString());
        ...
        it('Try to withdraw by Bob from empty collateral book, even with corrupted aggregator, expect revert', async () => {
            ...
            let bobMaxWithdraw = await currencyController.convertToETH(hexFILString, withdrawAmt);
        ...
        it('Try to withdraw by Bob from empty position with Carol, with corrupted aggregator, expect no balance changes', async () => {
            ...
            await collateral.setMaxCollateralWidthdraw(bob, carol, maxWithdraw, maxWithdraw);
            ...
            await vault.connect(bobSigner).withdrawFrom(carol, withdrawAmt.toString());
        ...
        it('Try to deposit by Bob into position with Carol and withdraw all amounts, validate correct balance changes', async () => {
            ...
            await tokenContract.approveInternal(bob, vault.address, bobTokenBalance.toString());
            await vault.connect(bobSigner)['deposit(address,uint256)'](carol, bobTokenBalance.toString());

            let bobMaxWithdraw = await currencyController.convertToETH(hexFILString, bobTokenBalance.toString());
            await collateral.setMaxCollateralWidthdraw(bob, carol, bobMaxWithdraw.toString(), '0');
            ...
            await vault.connect(bobSigner).withdrawFrom(carol, bobTokenBalance.toString());
