...
contract('BokkyPooBahsDateTimeContract', async (accounts) => {
    ...
    describe('Test functions library functions', async () => {
        ...
        it('Test basic time substraction functions', async () => {
            ...
            year.toString().should.be.equal(date.utc().subtract(1, 'y').year().toString());
            ...
            days.toString().should.be.equal(date.utc().subtract(15, 'd').date().toString());

            let subHours = await timeLibrary.subHours(time, 6);
            let hours = await timeLibrary.getHour(subHours);
            hours.toString().should.be.equal(date.utc().subtract(6, 'h').hour().toString());

            let subMinutes = await timeLibrary.subMinutes(time, 37);
            let minutes = await timeLibrary.getMinute(subMinutes);
            minutes.toString().should.be.equal(date.utc().subtract(37, 'm').minute().toString());
        });
        ...
        it('Test basic time difference functions', async () => {
            ...
            let diffYears = await timeLibrary.diffYears(dateNow.format("X"), shiftedTime.format("X"));
            diffYears.toString().should.be.equal(shiftedTime.diff(dateNow, 'years').toString());

            shiftedTime = date.add(2, 'M');
            let diffMonths = await timeLibrary.diffMonths(dateNow.format("X"), shiftedTime.format("X"));
            diffMonths.toString().should.be.equal(shiftedTime.diff(dateNow, 'months').toString());

            shiftedTime = date.add(20, 'd');
            let diffDays = await timeLibrary.diffDays(dateNow.format("X"), shiftedTime.format("X"));
            diffDays.toString().should.be.equal(shiftedTime.diff(dateNow, 'days').toString());

            shiftedTime = date.add(3, 'h');
            let diffHours = await timeLibrary.diffHours(dateNow.format("X"), shiftedTime.format("X"));
            diffHours.toString().should.be.equal(shiftedTime.diff(dateNow, 'hours', true).toString());

            shiftedTime = date.add(37, 'm');
            let diffMinutes = await timeLibrary.diffMinutes(dateNow.format("X"), shiftedTime.format("X"));
            diffMinutes.toString().should.be.equal(shiftedTime.diff(dateNow, 'minutes').toString());
        });
    });
});