...
describe('LendingMarketController - Orders', () => {
  ...
  describe('Orders', async () => {
    ...
    it('Check midUnitPrice without any orders', async () => {
      const midUnitPrice = await lendingMarketControllerProxy.getMidUnitPrice(
        targetCurrency,
        maturities[1],
      );
      expect(midUnitPrice).to.equal(5000);
    });

    it('Add orders and check midUnitPrice', async () => {
      ...
      expect(midUnitPrice).to.equal(9500);
    });
    ...
    it('Calculate the funds of users who have a large lending position and a small borrowing position', async () => {
      ...
      await lendingMarketControllerProxy
        .connect(owner)
        .createOrder(targetCurrency, maturities[0], Side.BORROW, '100', '8000');
      await lendingMarketControllerProxy
        .connect(owner)
        .createOrder(targetCurrency, maturities[0], Side.LEND, '100', '7999');
      ...
      await lendingMarketControllerProxy
        .connect(owner)
        .createOrder(targetCurrency, maturities[1], Side.BORROW, '100', '8000');
      await lendingMarketControllerProxy
        .connect(owner)
        .createOrder(targetCurrency, maturities[1], Side.LEND, '100', '7999');
